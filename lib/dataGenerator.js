...
const integerGenerator = ({ __max = 100, __min = 2 }) => {
  return randomNumberGenerator(__max, __min);
};

const floatGenerator = ({ __max = 100, __min = 2, __tofix = 2 }) => {
  return (randomNumberGenerator(__max, __min) * Math.random()).toFixed(__tofix);
};
...
const titleGenerator = ({ __max = 5, __min = 2, __limit = null }) => {
